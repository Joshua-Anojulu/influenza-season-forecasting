...
**Status: the pipeline is complete through notebook 08. Notebooks 01-07, all 14 `results/` artifacts, all 13 `figures/`, and the findings deck are on `main` and pushed to `origin/main`; notebook 08, `figures/12_D2_severity_heatmap.png`, and its six `results/08_*` artifacts landed as `e66c1e5` and `e71f5cb` and were pushed on Joshua's go-ahead on 2026-08-02, so `git rev-list --left-right --count origin/main...main` returns `0 0` again. The template-adherence pass landed as `e65cc1d` on 2026-07-21 after Claude independently re-ran the normalized protected gate (PASS 19/19) and reviewed the full diff; Act 3 of `PLAN-REVIEW-LOG.md` records the verdict. Results are preliminary and descriptive. Dr. Mitra signed off on framing, scope, and the regression/curve models on 2026-07-08, and on the D2 heatmap, severity tiers, and Option B on 2026-08-02 (see both "Confirmed by Dr. Mitra" sections); only the headline-W decision remains open below.**
...
**D2 is built (2026-08-02).** `figures/12_D2_severity_heatmap.png` plus `results/08_regional_severity.{json,md}`. The regional source is `data/raw/ILINet_regional.csv`, pulled from CDC FluView's own download endpoint (`gis.cdc.gov/grasp/flu2/PostPhase02DataDownload`, `RegionTypeId: 1` = HHS Regions, seasons 43-64), 10 regions x 1,148 weeks, `% WEIGHTED ILI` fully populated, identical schema and `skiprows=1` quirk to the national file. **Vintage was verified, not assumed:** re-pulling National from the same endpoint on the same day reproduced the local `ILINet.csv` with max absolute difference 0.0 across all 1,148 weeks, so regional and national data are the same revision. A Delphi Epidata mirror was tested first and REJECTED: 192 of 1,148 weeks differed by more than 0.001 with a max difference of 0.089, which exceeds the tier margin on three boundary-sensitive seasons and could have flipped a tier.

D2 is a **continuous** heatmap of regional season peak ILI. National IT50/IT90/IT98 appear as colorbar reference ticks only; no cell is colored or labeled by tier. Excluded seasons are marked with dotted columns and an asterisk.

**The regional diagnostic vindicates not tiering regionally, and is itself a finding.** Applying the national thresholds region-by-region would have encoded baseline geography as severity: Region 6's median season peak is 9.455, *above* the national IT98 of 8.6, and it exceeds IT98 in 14 of 21 assessed seasons, while Region 1's median of 3.896 sits below IT50. The exceedance table is in `results/08_regional_severity.md`, labeled diagnostic-only and never used to color the figure or label a season.
...
**Google Drive is a partial share mirror, not a mirror of `main`, and the folder names mislead.** The folder Dr. Mitra and two collaborators actually read is `Influenza Season Forecasting` (id `1shu3UBcQdomqO5uOCaA_4q9SSQV4DZKe`). Permissions confirmed 2026-08-02: `anyone` has `reader`, i.e. it is **publicly readable to anyone with the link**, plus three named writers (subhro.mitra@untdallas.edu, arindutta2020, zayanrschool). The folder named `Influenza Season Forecasting - shared` (id `1YCjOr5e_zq0k6V_Kz_hfMRMexEpKuYPw`) is private to Joshua and is a superseded 2026-07-08 copy.

**2026-08-02 partial Drive sync.** Added `CURRENT_STATE_2026-08-02.md` at the folder root (read it before the READMEs) and four markdown artifacts to `results/`: `08_tier_validation.md`, `08_severity_classifier.md`, `08_regional_severity.md`, `05_special_cases.md`. **Still missing from Drive:** the three PNGs (`10_D1`, `11_D3`, `12_D2`), notebooks 07 and 08, `07_features_summary.{json,md}`, `05_trajectories.json`, and the `.json` counterparts of the four files just added. Those are binaries or large machine artifacts and are faster to drag-drop than to push through the Drive API.

**The Drive tooling available here can create files but cannot update or delete them.** Re-uploading an existing filename produces a duplicate rather than a new version. The folder already carries two stale duplicate pairs that must be deleted by hand: `README.md` (two copies, both predating notebooks 07 and 08) and `results/05_survivorship.md` (two copies). Never "fix" a stale Drive file by uploading over it.
...
Open follow-ups (not yet done): empty `src/` with 02's logic duplicated across 03/04/05/06/07/08 (deferred refactor); D4, the research paper, unstarted.
...
National scope for every model. Regional data is a D2 figure input only. Raw files live in `data/raw/` (gitignored).
...
- **ILINet_regional.csv** (D2 figure input only, 2026-08-02): same schema and `skiprows=1` quirk as the national file, `REGION TYPE` = `HHS Regions`, 10 regions x 1,148 weeks, `% WEIGHTED ILI` fully populated. Pulled from FluView's download endpoint, not by hand. Vintage verified against the national file at max abs diff 0.0. Do NOT substitute a Delphi Epidata pull: it disagrees with CDC on 192 of 1,148 weeks by up to 0.089.
...
8. `08_severity_tiers.ipynb`: DONE. CDC-anchored severity tier definition, validation against CDC's published season classifications, the 3-class RF severity classifier with four baselines, and D2. The classifier is a negative result.
