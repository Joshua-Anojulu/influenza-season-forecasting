...
**Status: the committed pipeline remains complete through 06. Notebooks 01-06, the pre-existing `results/` and `figures/`, and the findings deck are on `main` and pushed to `origin/main`. The current working tree extends 05, adds one whitelisted persistence key to 06, and adds notebook 07 plus D1/D3 and new result artifacts. Those current changes are executed and verified but uncommitted pending Joshua's review. Results are preliminary and descriptive. Dr. Mitra signed off on framing, scope, and the regression/curve models on 2026-07-08 (see "Confirmed by Dr. Mitra"); the Option B and headline-W decisions remain open below.**

**2026-07-21 template-adherence pass (uncommitted).** Notebook 05 now persists 132 unique per-fit trajectories in a sidecar, scores the three excluded seasons separately as structural-break stress tests, and builds D1. Notebook 06 persists its already-computed standardized ridge coefficients under the sole whitelisted new JSON key. New notebook 07 adds the template lag, rolling, and hospitalization features; tests H1; builds D3 with grouped block ablations; and scores excluded seasons under Panel A. The protected gate passes 19/19 after line-ending normalization, with every pre-existing 06 JSON key unchanged. Notebook outputs are cleared before review.
...
Current working-tree extension (executed clean, outputs cleared, uncommitted pending Joshua's review):
...
Open follow-ups (not yet done): empty `src/` with 02's logic duplicated across 03/04/05/06/07 (deferred refactor); RF severity classifier remains not implemented and is superseded by the regression/curve and feature analyses.
...
7. `07_features_and_hypothesis.ipynb`: BUILT AND EXECUTED IN THE CURRENT WORKING TREE, UNCOMMITTED. Template lag/rolling/hospitalization features, H1 paired comparison, two provisional Option B ridge panels, grouped block ablations, excluded-season Panel A stress tests, and D3. H1 is not descriptively supported.
...
│   └── 07_features_and_hypothesis.ipynb  # uncommitted (template features + H1 + D3)
